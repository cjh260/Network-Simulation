...
function CanvasState(canvas) {
	...
	canvas.addEventListener('mousedown', function(e) {
	    ...
	    for (var i = l-1; i >= 0; i--) {
	    	if (devices[i].contains(mx, my)) {
	    		var mySel = devices[i];
	    		if(mySel.name === document.getElementById("currentDevice").value){
	...
	canvas.addEventListener('mouseup', function(e) {
		...
		var networks = myState.networks;
		var l = networks.length;
   		if(myState.deviceSelect){
   			for (var i = l-1; i >= 0; i--) {
				if (networks[i].contains(mouse.x, mouse.y)) {
					if(networks[i].name !== myState.selection.network){
						if(myState.selection.name === document.getElementById("currentDevice").value){
...
CanvasState.prototype.draw = function() {
	...
	if (!this.valid) {
	    ...
	    for (var i = 0; i < l; i++){
	    	...
	    	if(devices[i].name === document.getElementById("currentDevice").value){
		    	ctx.strokeStyle = "#FDCA40";
